...
const AdminOrders = () => {
  const [data, setData] = useState<StoredOrder[]>([]);
  const [filter, setFilter] = useState<Status | "all" | "unpaid">("all");

  const refresh = () => setData(ordersStore.all());
  useEffect(() => { refresh(); const i = setInterval(refresh, 2500); return () => clearInterval(i); }, []);
  ...
  const advance = (id: string, current: Status) => {
    ...
    ordersStore.update(id, { status: next });
    toast.success(`${id} → ${next}`);
    refresh();
  };
  const markPaid = (id: string) => {
    ordersStore.update(id, { payment_status: "paid", payment_ref: `UPI-${Date.now()}` });
    toast.success(`Payment confirmed for ${id}`);
    refresh();
  };
    ...
    <AdminLayout>
      <div className="space-y-6 animate-fade-up">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Verify UPI payments and update fulfillment status</p>
        </div>
              ...
              <div key={o.id} className="glass-card rounded-2xl p-5">
                ...
                <div className="flex flex-wrap gap-2 mt-4 pt-3 border-t border-border/40">
                  {o.payment_status === "pending" && (
                    <Button variant="aurora" size="sm" onClick={() => markPaid(o.id)} className="gap-1.5">
                      <CheckCircle2 className="h-4 w-4" /> Mark payment completed
                    </Button>
                  )}
                  <Button variant="glass" size="sm" disabled={o.status === "delivered"} onClick={() => advance(o.id, o.status)}>
                    Advance status
                  </Button>
                </div>
              </div>
