...
import { ArrowLeft, CheckCircle2, Loader2, ShoppingBag, Plus, Trash2, MapPin, CreditCard } from "lucide-react";
          ...
          <div className="glass-card rounded-3xl p-6 md:p-8 h-fit lg:sticky lg:top-24 space-y-6">
            ...
            <div className="flex flex-col items-center text-center space-y-4 pt-2">
              <div className="inline-flex items-center gap-1.5 text-xs font-medium uppercase tracking-wider text-primary">
                <CreditCard className="h-3.5 w-3.5" /> Secure Checkout
              </div>
              <div className="text-xs text-muted-foreground">Order ID: <span className="font-mono text-foreground">{orderId}</span></div>
              
              <Button onClick={payWithRazorpay} disabled={submitted || paying} variant="aurora" size="lg" className="w-full">
                {paying ? <><Loader2 className="h-4 w-4 animate-spin mr-2" /> Initializing...</>
                 : submitted ? <><CheckCircle2 className="h-4 w-4 mr-2" /> Order Placed</>
                 : <>Pay ₹{total.toFixed(0)} with Razorpay</>}
              </Button>
              
              {paidRef && <p className="text-[11px] text-emerald-500 font-mono">Payment Ref: {paidRef}</p>}
              <p className="text-[11px] text-muted-foreground">
                Payment is handled securely via Razorpay. Support for UPI, Cards, and Netbanking included.
              </p>
            </div>
          </div>
        </div>
